...
A CLI tool that enriches your coding agent prompts with relevant codebase context using a local LLM. No API keys, no cloud — runs entirely on your machine.

Designed to work as a **Claude Code plugin** — it hooks into your prompt submission flow and automatically enriches every prompt with codebase context before Claude sees it.

## Motivation

When you ask a coding agent like Claude Code, Cursor, or Copilot to work on your codebase, the agent spends significant time (and tokens) just discovering which files are relevant. It greps, reads, explores — all on your dime.

**promptscout** does that discovery work locally, for free. It uses a small local LLM to understand your prompt, searches your codebase with grep and git, and appends the discovered context to your original prompt. The paid agent receives a prompt that already contains the relevant file paths, code snippets, and commit history — so it can skip straight to the actual work.
...
On first run, promptscout will automatically download the Qwen 3 4B model (~2.5GB). The model is stored in `~/.promptscout/models/`.
...
promptscout ships with a Claude Code plugin that automatically enriches every prompt you send. Once installed, it runs in the background — no manual copy-pasting needed.

### Install the plugin

```bash
claude plugin add /path/to/promptscout/claude-plugin
```
...
If promptscout is not installed or fails for any reason, the plugin silently falls back and your original prompt goes through unchanged.
